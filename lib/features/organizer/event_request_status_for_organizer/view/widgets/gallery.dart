...
Widget buildGallerySectionOrganization(
    BuildContext context, List<String> media) {
  return Column(
    ...
    children: [
      ...
      Row(
        ...
        // mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          ...List.generate(media.length > 3 ? 3 : media.length, (index) {
            return (index == 2 && media.length > 3)
                ? Stack(
                    children: [
                      GalleryCard(
                        media: media,
                        imgUrl: "/storage/${media[index]}",
                      ),
                      GestureDetector(
                        onTap: () {
                          Get.toNamed('/GalleryScreen', arguments: {
...
class GalleryCard extends StatelessWidget {
  const GalleryCard({super.key, required this.imgUrl, required this.media});
  final String imgUrl;
